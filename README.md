...
This setup serves the frontend from **Cloudflare Pages** while proxying API/webhook paths to the backend, so the browser still uses a single origin (`https://pushook.com`).
...
4. Route the Worker to `pushook.com/*`.
5. On the backend, set `PUBLIC_BASE_URL=https://pushook.com`, `CORS_ORIGINS=https://pushook.com`, and `SERVE_FRONTEND=false`.
